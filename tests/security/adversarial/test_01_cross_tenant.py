"""01 - Cross-Tenant Attack: tenant A must not access tenant B data.

Enforces: P1 (tenant isolation), R3 (Cognitive Boundary).
"""
import sys
import uuid
from pathlib import Path

import pytest

sys.path.insert(0, str(Path(__file__).resolve().parents[4]))

from libs.access.rbac import ROLE_ADMIN, ROLE_SUPERADMIN, ROLE_VIEWER
from libs.access.security import JwtService
from libs.access.tenant_scope import AuthorizationContext, TenantScopeError

SECRET = "test-secret"
TENANT_A = str(uuid.uuid4())
TENANT_B = str(uuid.uuid4())


@pytest.fixture
def jwt():
    return JwtService(algorithm="HS256", secret_key=SECRET)


def _token(jwt, role, tenant_id):
    return jwt.create_access_token(
        user_id=str(uuid.uuid4()),
        tenant_id=tenant_id,
        email="attacker@test.com",
        role=role,
    )


def test_viewer_cross_tenant_denied(jwt):
    """Viewer from tenant A cannot access tenant B."""
    token = jwt.verify_access_token(_token(jwt, ROLE_VIEWER, TENANT_A))
    ctx = AuthorizationContext.from_token_payload(token)
    with pytest.raises(TenantScopeError):
        ctx.resolve(TENANT_B)


def test_admin_cross_tenant_denied(jwt):
    """Admin from tenant A cannot access tenant B."""
    token = jwt.verify_access_token(_token(jwt, ROLE_ADMIN, TENANT_A))
    ctx = AuthorizationContext.from_token_payload(token)
    with pytest.raises(TenantScopeError):
        ctx.resolve(TENANT_B)


def test_superadmin_cross_tenant_allowed(jwt):
    """Superadmin CAN access any tenant."""
    token = jwt.verify_access_token(_token(jwt, ROLE_SUPERADMIN, TENANT_A))
    ctx = AuthorizationContext.from_token_payload(token)
    effective = ctx.resolve(TENANT_B)
    assert effective == TENANT_B


def test_same_tenant_always_allowed(jwt):
    """Any role accessing its own tenant is allowed."""
    for role in (ROLE_VIEWER, ROLE_ADMIN, ROLE_SUPERADMIN):
        token = jwt.verify_access_token(_token(jwt, role, TENANT_A))
        ctx = AuthorizationContext.from_token_payload(token)
        effective = ctx.resolve(TENANT_A)
        assert effective == TENANT_A
